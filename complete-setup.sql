...
-- Modules: Read-only for authenticated users
...
-- User permissions: Users can view own permissions, admins can manage all
...
-- 8. Create function to create user permissions from role template
...
-- 10. Add updated_at trigger for user_permissions
DROP TRIGGER IF EXISTS update_user_permissions_updated_at ON user_permissions;
CREATE TRIGGER update_user_permissions_updated_at
    BEFORE UPDATE ON user_permissions
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 11. Update existing user profiles to have is_active = true
UPDATE user_profiles SET is_active = true WHERE is_active IS NULL;

-- 12. Update existing user profiles to have attributes = {} if null
UPDATE user_profiles SET attributes = '{}' WHERE attributes IS NULL;

-- Success message
SELECT 'User Management System setup completed successfully!' as status; 