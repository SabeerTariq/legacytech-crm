...
async function runSQLMigration() {
  console.log('🚀 Running SQL Migration for Role Management...');
  ...
  try {
    ...
    for (let i = 0; i < sqlQueries.length; i++) {
      ...
      console.log(`Executing query ${i + 1}/${sqlQueries.length}...`);
      ...
      try {
        ...
        if (error) {
          console.log(`Query ${i + 1} failed with rpc:`, error.message);
          // Try alternative approach - this will fail for DDL but we'll catch it
          console.log(`Skipping query ${i + 1} (DDL not supported via client)`);
        } else {
    ...
    console.log('Note: Some DDL operations may need to be run manually in the Supabase SQL editor.');
