-- Fix the team performance function to use custom email system from user management
-- This will use the email created from user management as the main email for user and employee linking

-- Drop the existing function if it exists
DROP FUNCTION IF EXISTS get_team_performance_summary(p_month DATE);

-- Create the updated function that uses user management emails as primary linking
...
BEGIN
  ...
  -- Join with employees using the user management email as the primary link
...
SELECT 'Testing updated team performance function with user management email linking...' as status;
