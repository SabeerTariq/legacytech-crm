...
async function extractDataFromSupabase(tableName) {
  ...
  try {
    // Try to order by created_at first, then by id if that fails
    ...
    try {
      ...
      try {
        ...
        console.log(`  ⚠️  Could not order by created_at or id, using no ordering`);
      }
    }
...
async function migrateFinalBatch() {
  console.log('🚀 Starting migration of final batch of tables...\n');
  ...
  try {
    ...
    // Final batch of tables to migrate
    ...
    console.log('\n✅ Final batch migration completed!');
