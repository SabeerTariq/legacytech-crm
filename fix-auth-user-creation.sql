...
-- This script identifies and fixes issues preventing user creation in Supabase Auth

-- 1. Check for any triggers that might be causing issues
DO $$
DECLARE
...
BEGIN
    ...
    FOR trigger_record IN 
        ...
        AND event_object_table IN ('user_profiles', 'employees', 'user_roles')
    LOOP
        RAISE NOTICE 'Found trigger: % on table %', trigger_record.trigger_name, trigger_record.event_object_table;
    END LOOP;
END $$;

-- 2. Check for any functions that might be called during user creation
DO $$
DECLARE
    func_record RECORD;
BEGIN
...
-- 3. Check for any RLS policies that might be causing issues
DO $$
DECLARE
    policy_record RECORD;
BEGIN
    RAISE NOTICE 'Checking RLS policies that might interfere...';
    ...
    FOR policy_record IN 
        ...
        AND tablename IN ('user_profiles', 'employees', 'user_roles')
    LOOP
...
BEGIN
    ...
    FOR fk_record IN 
        ...
        AND tc.table_name IN ('user_profiles', 'employees', 'user_roles')
    LOOP
...
-- 5. Check if there are any check constraints that might be causing issues
DO $$
DECLARE
    check_record RECORD;
BEGIN
...
-- 6. Check for any unique constraints that might be causing issues
DO $$
DECLARE
...
-- 7. Check if there are any missing tables that might be referenced
DO $$
DECLARE
    missing_table TEXT;
BEGIN
...
-- 8. Check if there are any enum types that might be causing issues
DO $$
DECLARE
    enum_record RECORD;
BEGIN
...
BEGIN
    ...
    LOOP
        RAISE NOTICE 'Found foreign table: %', foreign_table_record.ftrelid;
    END LOOP;
END $$;
